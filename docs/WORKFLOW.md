...
## 7. 개발 착수 핸드오프 (PM → 개발 창) — 운영 규칙

> 기획만 계속하지 않는다. 어떤 범위가 **개발 가능(Go) 상태**가 되면 곧바로 개발과 **병행**한다.
> PM 창은 어떤 범위가 Go가 되는 순간 아래를 한 번에 안내한다.

1. **Go 판정 + 범위 공지** — PLAN의 '개발 준비도(Go/No-Go)' 게이트로 "지금 개발 가능한 범위"를 명시.
2. **개발 창 생성 안내** — 사용자에게 "새 개발 대화창을 열라"고 안내.
3. **시작 프롬프트 제공** — 그 개발 창에 **복사-붙여넣기할 시작 프롬프트**를 통째로 제공(읽을 문서 + 구현 범위 + 루프 + 완료기준 포함, 자체 완결적으로).
4. **머지까지 진행** — 개발 창은 표준 루프(2번)대로 **브랜치→PR→Preview→머지**까지 진행해 사용자가 라이브로 바로 확인.
5. **다음 기획 제안** — PM 창은 개발이 도는 동안 **다음에 이어갈 기획**을 제안하고 계속 진행.

> ⚠️ **전제:** 개발 창은 새 세션이라 보통 `main`을 클론한다. PM의 PLAN 갱신이 아직 `main`에 없으면 개발 창이 최신 스펙을 못 본다. → **PM 문서를 먼저 `main`에 반영**하거나(문서는 PR 없이 main 직접 커밋 허용, 1번 원칙), 시작 프롬프트를 **자체 완결적**으로 작성한다.
